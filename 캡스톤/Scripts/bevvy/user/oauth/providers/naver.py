...
class NaverLoginMixin:
    ...
    def login_with_naver(self, state, code):
        ...
        user.sex = profiles.get('gender')
        user.age = profiles.get('age')
        user.profile_image = profiles.get('profile_image')
        user.is_active = True
        user.save()

        # 로그인
        login(self.request, user)
        # login(self.request, user, 'user.oauth.backends.NaverBackend') 이전거 but 에러남 먼가 문제가 있음
